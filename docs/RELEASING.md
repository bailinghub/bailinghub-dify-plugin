...
5. Install the generated `.difypkg` into a test Dify workspace.
6. Validate credentials with a dedicated test client.
7. Run one harmless route through `queued -> done` and preserve a sanitized result.
8. Publish the plugin package and release notes in this repository only.
9. Update any optional link in BailingHub documentation after the adapter release exists.

## Distribution Stages

1. Local `.difypkg` for maintainer verification.
2. Independent GitHub repository for versioned community use.
3. Dify Marketplace submission after independent installation evidence and documentation
